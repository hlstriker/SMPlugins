...
new const String:PLUGIN_NAME[] = "Zone Type: Timer End Start";
...
public Plugin:myinfo =
{
	name = PLUGIN_NAME,
	author = "hlstriker",
	description = "A zone type of timer end start.",
	version = PLUGIN_VERSION,
	url = "www.swoobles.com"
}

new const String:SZ_ZONE_TYPE_NAME[] = "Timer End Start";
...
{
	CreateConVar("zone_type_timer_end_start_ver", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_PRINTABLEONLY);
...
{
	ZoneManager_RegisterZoneType(ZONE_TYPE_TIMER_END_START, SZ_ZONE_TYPE_NAME, OnTouch, OnStartTouch, OnEndTouch, OnEditData, OnTypeAssigned);
}
...
public OnStartTouch(iZone, iOther)
{
	...
	SpeedRuns_ClientTouchEnd(iOther, ZoneManager_GetDataInt(iZoneID, 1), false);
	SpeedRuns_TryStageFailed(iOther, ZoneManager_GetDataInt(iZoneID, 1) + 1);
}
...
public OnEndTouch(iZone, iOther)
{
	...
	SpeedRuns_ClientTouchStart(iOther, ZoneManager_GetDataInt(iZoneID, 1) + 1, iZoneID);
}
...
DisplayMenu_EditData(iClient, iZoneID)
{
	...
	switch(g_iEditingType[iClient])
	{
		...
		{
			new iData = ZoneManager_GetDataInt(iZoneID, 1);
			FormatEx(szTitle, sizeof(szTitle), "Edit End Start Number\n \nEnd set to: %i\nStart set to: %i\n \nType the number in chat.", iData, iData + 1);
		}
		...
		{
			ZoneManager_GetDataString(iZoneID, 1, szTitle, sizeof(szTitle));
		...
		case MENU_INFO_SET_TELEPORT_DEST_NAME:
		{
			...
			ZoneManager_GetDataString(iZoneID, 4, szTitle, sizeof(szTitle));
			...
			if(!bHasCustomOrigin)
				ZoneManager_GetDataString(iZoneID, 2, szTitle, sizeof(szTitle));
	...
	AddMenuItem(hMenu, szInfo, "Edit end start number.");
...
public MenuHandle_EditData(Handle:hMenu, MenuAction:action, iParam1, iParam2)
{
	...
	if(iType == MENU_INFO_SET_LINES)
	{
		...
		DisplayMenu_LineType(iParam1, g_iEditingZoneID[iParam1]);
		return;
	}
...
DisplayMenu_LineType(iClient, iZoneID)
{
	new Handle:hMenu = CreateMenu(MenuHandle_LineType);
	SetMenuTitle(hMenu, "Select Line Type");
	
	AddMenuItem(hMenu, "0", "Start Line");
	AddMenuItem(hMenu, "1", "End Line");
	
	SetMenuExitBackButton(hMenu, true);
	if(!DisplayMenu(hMenu, iClient, 0))
	{
		PrintToChat(iClient, "[SM] Error displaying menu.");
		OnEditData(iClient, iZoneID);
		return;
	}
...
public MenuHandle_LineType(Handle:hMenu, MenuAction:action, iParam1, iParam2)
{
	if(action == MenuAction_End)
	{
		CloseHandle(hMenu);
	...
	new LineType:iLineType = LineType:StringToInt(szInfo);
	StartEndLines_DisplayLineMenu(iParam1, g_iEditingZoneID[iParam1], iLineType, OnEditData);
}
...
public OnClientSayCommand_Post(iClient, const String:szCommand[], const String:szArgs[])
{
	...
	switch(g_iEditingType[iClient])
	{
		case MENU_INFO_SET_NUMBER:
		{
			...
			{
				PrintToChat(iClient, "[SM] Error: Must be between 1 and %i.", MAX_STAGES - 1);
				return;
			}
			
			ZoneManager_SetDataInt(g_iEditingZoneID[iClient], 1, iInt);
			PrintToChat(iClient, "[SM] Set end to %i and start to %i.", iInt, iInt + 1);
		}
		case MENU_INFO_SET_SPEED_CAP:
		{
			new iInt = StringToInt(szArgs);
		...
		case MENU_INFO_SET_TELEPORT_DEST_NAME:
		{
			...
			if(!szDataString[0] || StrEqual(szDataString, "-1"))
			{
				ZoneManager_SetDataString(g_iEditingZoneID[iClient], 2, "");
				ZoneManager_SetDataString(g_iEditingZoneID[iClient], 4, "");
			...
			else
			{
				if(StrEqual(szDataString, "<pos>", false))
				{
					...
					ZoneManager_SetDataString(g_iEditingZoneID[iClient], 4, szDataString);
					PrintToChat(iClient, "[SM] Set teleport position to: %i, %i, %i.", RoundFloat(fOrigin[0]), RoundFloat(fOrigin[1]), RoundFloat(fOrigin[2]));
				}
				else
				{
					ZoneManager_SetDataString(g_iEditingZoneID[iClient], 4, szDataString);
					ZoneManager_SetDataString(g_iEditingZoneID[iClient], 2, szDataString);
