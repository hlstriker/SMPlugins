...
new Float:g_fTimeStartedFirst[MAXPLAYERS+1];
new Float:g_fTimeStartedCurrent[MAXPLAYERS+1];

new g_iDemoTickStarted[MAXPLAYERS+1][MAX_STAGES+1];

new bool:g_bIsPaused[MAXPLAYERS+1];
new Float:g_fPausedStartTime[MAXPLAYERS+1];
new Float:g_fPausedTimeTotal[MAXPLAYERS+1];
new Float:g_fPausedTimeStage[MAXPLAYERS+1];
new Float:g_fPausedOrigin[MAXPLAYERS+1][3];
new Float:g_fPausedAngles[MAXPLAYERS+1][3];
new Float:g_fPausedVelocity[MAXPLAYERS+1][3];
new Float:g_fPausedVelocityBase[MAXPLAYERS+1][3];
new Float:g_fPausedVelocityFall[MAXPLAYERS+1];
new Float:g_fPausedVelocityModifier[MAXPLAYERS+1];
...
Float:GetAverageSpeed(iClient, bool:bGetStageSpeed=true)
{
	...
	if(bGetStageSpeed)
	{
		fSpeed = (g_fTotalSpeedStage[iClient] * 1000000) / g_iTotalSpeedStageCount[iClient];
	}
	else
	{
		fSpeed = (g_fTotalSpeedMap[iClient] * 1000000) / g_iTotalSpeedMapCount[iClient];
	}
	
	return fSpeed;
}

AddToTotalSpeed(iClient)
{
	if(g_bIsPaused[iClient])
		return;
	...
	fSpeed = GetVectorLength(fVelocity) / 1000000; // Divide by a million in order to add many numbers to the total. Multiply by a million when getting the speed.
	
	g_fTotalSpeedMap[iClient] += fSpeed;
	g_fTotalSpeedStage[iClient] += fSpeed;
	
	g_iTotalSpeedMapCount[iClient]++;
	g_iTotalSpeedStageCount[iClient]++;
}
...
StageStart(iClient, iStageNumber, iZoneID)
{
	...
	g_fTimeStartedCurrent[iClient] = GetGameTime();
	g_fPausedTimeStage[iClient] = 0.0;
	...
	if(iStageNumber == 1)
	{
		...
		g_fTimeStartedFirst[iClient] = g_fTimeStartedCurrent[iClient];
		g_fPausedTimeTotal[iClient] = 0.0;
...
public APLRes:AskPluginLoad2(Handle:hMyself, bool:bLate, String:szError[], iErrLen)
{
	...
	CreateNative("SpeedRuns_PauseRun", _SpeedRuns_PauseRun);
...
public _SpeedRuns_IsRunPaused(Handle:hPlugin, iNumParams)
{
	if(iNumParams != 1)
		SetFailState("Invalid number of parameters SpeedRuns_IsRunPaused");
	
	return g_bIsPaused[GetNativeCell(1)];
}

public _SpeedRuns_PauseRun(Handle:hPlugin, iNumParams)
{
	if(iNumParams != 2)
		SetFailState("Invalid number of parameters SpeedRuns_PauseRun");
	
	new iClient = GetNativeCell(1);
	new bool:bShouldPause = GetNativeCell(2);
	
	// Is it already paused or unpaused?
	if(bShouldPause == g_bIsPaused[iClient])
		return false;
...
PauseRun(iClient)
{
	g_fPausedStartTime[iClient] = GetGameTime();
	
	GetClientAbsOrigin(iClient, g_fPausedOrigin[iClient]);
	GetClientEyeAngles(iClient, g_fPausedAngles[iClient]);
	
	GetEntPropVector(iClient, Prop_Data, "m_vecVelocity", g_fPausedVelocity[iClient]);
	GetEntPropVector(iClient, Prop_Send, "m_vecBaseVelocity", g_fPausedVelocityBase[iClient]);
	g_fPausedVelocityFall[iClient] = GetEntPropFloat(iClient, Prop_Send, "m_flFallVelocity");
	g_fPausedVelocityModifier[iClient] = GetEntPropFloat(iClient, Prop_Send, "m_flVelocityModifier");
	
	g_bIsPaused[iClient] = true;
}

UnpauseRun(iClient)
{
	TeleportEntity(iClient, g_fPausedOrigin[iClient], g_fPausedAngles[iClient], g_fPausedVelocity[iClient]);
	SetEntPropVector(iClient, Prop_Send, "m_vecBaseVelocity", g_fPausedVelocityBase[iClient]);
	SetEntPropFloat(iClient, Prop_Send, "m_flFallVelocity", g_fPausedVelocityFall[iClient]);
	SetEntPropFloat(iClient, Prop_Send, "m_flVelocityModifier", g_fPausedVelocityModifier[iClient]);
	
	new Float:fPausedTime = GetGameTime() - g_fPausedStartTime[iClient];
	g_fPausedTimeStage[iClient] += fPausedTime;
	g_fPausedTimeTotal[iClient] += fPausedTime;
	
	g_bIsPaused[iClient] = false;
}
