...
new const String:PLUGIN_NAME[] = "[UltJB] LR: Contest - Gun Toss Old School";
...
public Plugin:myinfo =
{
	name = PLUGIN_NAME,
	author = "hlstriker",
	description = "Last Request: Contest - Gun Toss Old School.",
	version = PLUGIN_VERSION,
	url = "www.swoobles.com"
}

#define LR_NAME			"Gun Toss: Old School"
...
new Float:g_fLastGroundOrigin[MAXPLAYERS+1][3];


public OnPluginStart()
{
	CreateConVar("lr_contest_gun_toss_os_ver", PLUGIN_VERSION, PLUGIN_NAME, FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_PRINTABLEONLY);
}
...
SetStartPosition(iClient)
{
	// If they are in the air get their last ground position.
	if(GetEntPropEnt(iClient, Prop_Send, "m_hGroundEntity") == -1)
	{
		g_fPositionOrigin[POSITION_START][iClient][0] = g_fLastGroundOrigin[iClient][0];
		g_fPositionOrigin[POSITION_START][iClient][1] = g_fLastGroundOrigin[iClient][1];
		g_fPositionOrigin[POSITION_START][iClient][2] = g_fLastGroundOrigin[iClient][2];
	}
	else
	{
...
{
	if(GetEntPropEnt(iClient, Prop_Send, "m_hGroundEntity") != -1)
		GetClientAbsOrigin(iClient, g_fLastGroundOrigin[iClient]);
